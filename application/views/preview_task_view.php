	...
	<div style="clear:both;padding-top:10px">
		<table>
			...
			<tr>
				...
				<td>:</td>
				<td>
					<span class="label label-primary"><?php echo $_TASK_DATA->user['fullname']; ?></span>
				</td>
			</tr>
		</table>
	</div>
...
<div id="task_member" style="display:none">
	<?php //לולאה שמציגה את כל הפעילים במשימה הנוכחית בפרויקט
	$allowAdd = array();
	foreach ($_TASKER_DATA as $key => $value) {
		array_push($allowAdd, $value->user['id']);
		echo '<div class="icon-user user-project n-button" onClick="userPreview(\''.$value->user['id'].'\')" >'.$value->user['fullname'].'</div>'; //מעבר בקליק לפרופיל של המשתמש
	}
	?>
</div>
		...
		<?php foreach ($_ACT_DATA as $key => $value) { ?>
			<tr>
				...
				<td  class="activity-td">
					<div style="border:3px solid #DFDFDF;float:left;width:100%">

						<table style="width:100%">
							<tr>
								<td style="height:20px">
									<div class="margined label label-default" style="float:left" >
										#<?php echo $value['id']; //הצגת מספר המשימה ?>
									</div>
									<div class="icon-user user-project" style="float:left" >
										<span class="label label-primary"><?php echo $_TASK_DATA->user['fullname']; ?></span>
										<?php if($value['user_id'] == Handler::$_LOGIN_USER_ID){ //אם המשתמש שבמחובר הוא זה שהוסיף את ההתקדמות יש לו אופציית עריכה ומחיקה ?>
											<a href="#" onClick="editActivity('<?php echo $value['id']; ?>')">[Edit]</a> <a href="#" onClick="deleteActivity('<?php echo $value['id']; ?>')">[Delete]</a>
										<?php } ?>
									</div>
								</td>
								<td rowspan="2" style="width:175px;vertical-align:top">
									<table>
										<tr>
											<td>
												<div class="icon-date icon-enabled" style="float:left;margin:20px"><?php echo date(Handler::$_DF,strtotime($value['input_date'])); //הצגת תאריך ?></div>
											</td>
										</tr>
										<tr>
											<td>
												<div class="<?php echo $value->status['icon']; ?> icon-enabled" style="float:left;margin:5px"><div><span class="label label-info"><?php echo $value->status['name']; //סטטוס ?> </span>
												</div>
											</div>
										</td>
									</tr>
						...
						<tr>
							<td style="vertical-align:top;background-color: #d2e1f161;vertical-align:top;">
...
$(function () {
	...
	<?php if($_TASK_DATA->user['id'] == Handler::$_LOGIN_USER_ID){?> //אם המשימה היא של המשתמש יש להציג את פקד העריכה
		var descTitle = 'Description <a id="edit_button_owner" href="#" onClick="editDesc()">[Edit]</a>' ;
		<?php } else {?>
			var descTitle = 'Description' ; //אחרת לא יוצג פקד
			<?php } ?>
				...
				$().w2form({
					...
					actions: {
						"Save": function () {
							var form = this;
							if(this.validate() == ''){
								w2confirm('Are you sure you want to post the progress?', function (btn) {
									if(btn == 'Yes'){
										form.save();
									}
								});
					...
					onSave: function(event) {
						w2popup.close();
						location.reload();
						showMessage('Progress added successfully.','success') ;
					}
				});
			}
			$().w2popup('open', {
				...
				$().w2form({
					...
					actions: {
						"Save": function () {
							var form = this;
							if(this.validate() == ''){
								w2confirm('Update changes?', function (btn) {
									if(btn == 'Yes'){
										form.save();
									}
								});
					...
					onSave: function(event) {
						w2popup.close();
						showMessage('Progress has been successfully updated.','success') ;
						location.reload();
					}
				});
			}
		...
		function deleteActivity(id) {
			w2confirm('Are you sure you want to delete the progress?', function (btn) {
				if(btn == 'Yes'){
					$.ajax({
						...
						showMessage('Progress deleted successfully.','success') ;
						location.reload();
					});
				}
			});
		...
		function openEditor(){
			...
			function saveDesc() {
				$.ajax({
					...
					showMessage('Progress description successfully updated.','success') ;
					location.reload();
				});
			}
