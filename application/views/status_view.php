<?php include('header.php'); ?>
    ...
    <div id="content">
	 	<?php if(Handler::$_IS_ADMIN || Handler::$_HAVE_PT_STATUS){  ?>
...
<script type="text/javascript">
	...
	$(function () {
        $('#user_group_grid').w2grid({ 
	        ...
	        url: '<?php echo BASE_URL; ?>status/get_status_list',
	        method: 'POST',
	        recid:'id',
	       	header:'Status Control Center',
        	multiSelect			: false, 
		    ...
		    toolbar: {
		        items: [
					...
					{ type: 'drop',  id: 'User_info', caption: 'Information', img: 'icon-info',
						html: '<div style="padding: 10px; line-height: 1.5">You are in the Status table.'
					},
		        ],
		        onClick: function (target, data) {
		        	...
		        	if(target == 'edit_user_group'){
		        		if(selected == null){
						    showMessage('Please select a status to edit.','error') ;
		        		} else {
		        	...
		        	if(target == 'delete_user_group'){
		        		if(selected == null){
						    showMessage('Please select a status to delete.','error') ;
		        		} else {
	...
	function addStatus(){
    	...
    	if (!w2ui.new_user_group_form) {
	        $('#FORM_ADD_PROJECT').w2form({
		        ...
		        url: '<?php echo BASE_URL; ?>status/go_insert_status',
	            fields: [
	        		{ 
					    name     : 'name',      
					    type     : 'text',       
			    ...
			    onSave: function(event, data) {
			    	if(data.xhr.responseText == "0"){
		    			w2alert('Status name already taken');
			    	} else {
			        w2popup.close();
	...
	function editUserGroup(id){
		var getUrl = '<?php echo BASE_URL; ?>status/get_status/'+id;
		var saveUrl = '<?php echo BASE_URL; ?>status/go_edit_status/'+id;
