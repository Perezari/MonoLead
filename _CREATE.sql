...
CREATE TABLE `user` (
  ...
  `PER_USERS_TABLES` enum('Active','Nonactive') DEFAULT NULL,
  `PER_USERSGROUPS_TABLES` enum('Active','Nonactive') DEFAULT NULL,
  `PER_PROJECTS_TABLES` enum('Active','Nonactive') DEFAULT NULL,
  `PER_TASKS_TABLES` enum('Active','Nonactive') DEFAULT NULL,
  `PER_STATUS_TABLES` enum('Active','Nonactive') DEFAULT NULL,
  `PER_LOG_TABLES` enum('Active','Nonactive') DEFAULT NULL,
...
(4, 'CPPEXP', 'CPPEXP', 'CPPEXP', 'icon-CPPExpert');

CREATE VIEW `usergroup_view` as SELECT
  usergroup.id,
  usergroup.groupcode,
  usergroup.usergroup,
  usergroup.badge,
  usergroup.icon,
  COUNT(user.usergroup_id) AS Total
FROM
  user
RIGHT JOIN usergroup ON user.usergroup_id = usergroup.id
